...
{  
    public class MedicalRecordSVC
    {
        ...
        public List<Consultation> GetConsultationsForPatient(int patientId)
        {
            ...
            string query = @"SELECT mh.MedicId AS IdMedic, mh.PatientId AS IdPatient, m.Name AS MedicName, m.LastName AS MedicLastName, s.SpecialityName AS Specialty, m.MedicalRegistration AS MedicalRegistration,
                             mh.CreatedAt AS Date, mh.Reason, mh.Diagnosis, mh.MedicalHistoryId
                             FROM MedicalHistory mh
                             JOIN Medics m ON mh.MedicId = m.MedicId
                             JOIN Specialty s ON m.SpecialtyId = s.SpecialtyId
            ...
            try
            {
                ...
                if (result != null && result.Rows.Count > 0)
                {
                    foreach (DataRow row in result.Rows)
                    {
                        ...
                        consultation.MedicLastName = row["MedicLastName"].ToString();
                        consultation.MedicSpecialty = row["Specialty"].ToString();
                        consultation.MedicRegistration = row["MedicalRegistration"].ToString();
